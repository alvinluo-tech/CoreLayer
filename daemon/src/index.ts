...
import { seedDefaultAgent } from "./bootstrap/seed-agent-profiles.js";
...
// Seed default agent profiles before exposing the daemon or background workers.
void seedDefaultAgent().then(() => {
...
startBackgroundServices().catch((err) => console.error("[Jarvis] Background services startup failed:", err));
  ...
  console.error("[Jarvis] Daemon bootstrap failed:", err);
  process.exit(1);
});
