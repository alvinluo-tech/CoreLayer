...
export function JarvisVoiceOverlay({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const speedFactor =
        state === 'listening' || state === 'speaking' ? Math.max(0.3, volumeFactor) : 1.0;
      ...
      for (let i = 0; i < config.waves; i++) {
        ...
        for (let x = 0; x < w; x++) {
          ...
          const y =
            h / 2 + Math.sin(x * currentFreq + phaseRef.current + i * 1.5) * currentAmp * edgeFade;
      ...
      >
        ...
        <div
          className={cn(
            isCentered
              ? 'space-y-6'
              : 'flex-1 flex flex-col justify-between overflow-hidden mt-3 min-h-0'
          )}
        >
          ...
          <div
            className={cn(
              'space-y-3',
              !isCentered ? 'flex-1 overflow-y-auto min-h-0 py-2 pr-1' : ''
            )}
          >
