...
export function createConnectRealtimeSession(
  ...
  return async () => {
    ...
    if (refs.peerConnectionRef.current) {
      try { refs.peerConnectionRef.current.close(); } catch (e) { logger.debug("[VoiceConversation] peer connection close ignored:", e); }
      refs.peerConnectionRef.current = null;
    }
    if (refs.localStreamRef.current) {
      try { refs.localStreamRef.current.getTracks().forEach(t => t.stop()); } catch (e) { logger.debug("[VoiceConversation] local stream stop ignored:", e); }
