...
describe("useModelStore", () => {
  describe("fetchAll", () => {
    it("populates all data when all calls succeed", async () => {
      ...
      mockListProviderPresets.mockResolvedValueOnce({
        presets: [{ id: "openai", name: "OpenAI", nameCN: "", type: "openai_compatible", defaultBaseURL: "https://api.openai.com/v1", requiresApiKey: true, popularModels: [] }],
      });
    ...
    it("handles array-format providers response", async () => {
      mockGetProviderConfigs.mockResolvedValueOnce({
        providers: [
          { id: "openai", name: "OpenAI", type: "openai_compatible", baseURL: "https://api.openai.com/v1", apiKey: "sk-***", enabled: true },
        ],
      });
      mockListProviderPresets.mockResolvedValueOnce({ presets: [] });
      mockGetRoutingRules.mockResolvedValueOnce({ rules: [], isCustom: false });
  ...
  describe("addProvider", () => {
    ...
    it("calls addProvider and refreshes on success", async () => {
      const preset = { id: "openai", name: "OpenAI", nameCN: "", type: "openai_compatible" as const, defaultBaseURL: "https://api.openai.com/v1", requiresApiKey: true, popularModels: [] };
      ...
      mockGetProviderConfigs.mockResolvedValueOnce({ providers: { openai: { apiKey: "sk-***", baseURL: "https://api.openai.com/v1" } } });
